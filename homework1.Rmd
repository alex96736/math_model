 ...
 **X∼U(5,105)X∼U(5,105)  
 Y=f(X)+ϵY=f(X)+ϵ**  
 где f(x)=12-0.05x, ϵ∼N(0,1)ϵ∼N(0,1)
...
- При движении слева направо MSE на тестовой выборке (красная кривая) плавно растёт. Нам известна истинная форма связи Y с X, она описывается прямой. Число степеней свободы такой модели равно числу оцениваемых параметров, т.е. 2 (коэффициенты перед X, и константа). Рост MSE на тестовой выборке объясняется эффектом переобучения модели: она всё лучше описывает обучающую выборку, и при этом постепенно становится неприменимой ни к одному другому набору наблюдений.
Наименьшее значение MSE на тестовой выборке соответствует числу степеней свободы 2 и равно 1.33. Что и является компромиссом между простотой и точностью модели.
...
Теперь изменим исходные данные, пусть количество наблюдений изменится с 60 до 300.
...
# легенда
legend('bottomleft', legend = c('обучающая', 'тестовая'),
       pch = c(NA, 16), 
       col = c(grey(0.2), 'red'),  
       lty = c(1, 1), lwd = c(2, 2), cex = 1.2)
...
Сравнив полученные модели, можно увидеть, что при увеличении количества наблюдений, лучшая модель ближе к истинной функции, так же значение ошибки MSE на тестовой выборке уменьшается, в данном случае она изменилась с 1.33 до 1.21
